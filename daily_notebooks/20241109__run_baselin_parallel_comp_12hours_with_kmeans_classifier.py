...
def run_models(file_mappings):
    ...
    for i in tqdm(range(len(file_mappings))):
        ...
        bd_preds_classed = batdetect2_pipeline.classify_calls_from_file(bd_annotations_df, cur_seg['audio_seg'])
...
def apply_model(file_mapping):
    ...
    bd_preds_classed = batdetect2_pipeline.classify_calls_from_file(bd_dets, file_mapping['audio_seg'])
...
def apply_models(file_path_mappings):
...
if __name__ == '__main__':
    ...
    bd2_dets.to_csv('20241108__xl_160gb_baseline_8p1c1t_kmeansbd2preds.csv', index=False)
    delete_segments(segmented_file_paths)
    assert(len(list(cfg['tmp_dir'].iterdir()))==0)
    ...
    np.save(f'{Path(__file__).parent}/20241109__xl_160gb_baseline_8p1c1t_kmeansbd2_estimates.npy', baseline_estimates)
    ...
    classified_preds = apply_models(file_path_mappings)
    classified_preds.to_csv('20241108__xl_160gb_parallel_8p1c1t_kmeansbd2preds.csv', index=False)
    num_processes = multiprocessing.cpu_count()
    torch.set_num_threads(1)
    ctx = multiprocessing.get_context("spawn")
    pool = ctx.Pool(processes=num_processes)
    ...
    np.save(f'{Path(__file__).parent}/20241109__xl_160gb_parallel_8p1c1t_kmeansbd2_estimates.npy', parallel_estimates)
